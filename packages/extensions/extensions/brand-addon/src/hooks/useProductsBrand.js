 ...
 *
 * @param {object}      props
 * @param {number}      props.id - Brand Id.
 ...
 */
export const useProductsBrand = props => {
    const {
        id,
        queries: { getPageSize }
    } = props;
    ...
    const { getProductFiltersByBrandQuery, getFilterInputsQuery } = operations;
    ...
    const [runQuery, queryResponse] = useLazyQuery(getProductFiltersByBrandQuery, {
        fetchPolicy: 'cache-and-network',
        nextFetchPolicy: 'cache-first'
    });
    const {
    ...
    useEffect(() => {
        ...
        // Use the brand id for the current brand page regardless of the
        // applied filters. Follow-up in PWA-404.
        // newFilters['brand_id'] = { eq: String(id) };
    ...
    const productsBrandData = brandLoading && !data ? null : data;
    const metaDescription =
        data && data.lofBrandById && data.lofBrandById.meta_description
            ? data.lofBrandById.meta_description
            : '';
